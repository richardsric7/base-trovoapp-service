...
func SubmitXdrWithSignature(client *horizonclient.Client, ownerPublicKey string, xdrBase64 string, signature string) (string, error) {
	// fallback default, used only when EXPANSION_NETWORK_ERROR_WEBHOOK isn't configured for this environment.
...
func logDiscordFailedPayment(msg string) {
	// fallback default, used only when FAILED_PAYMENT_ERROR_WEBHOOK isn't configured for this environment.
	// NOTE: this webhook has been committed to source history - rotate it in Discord's channel settings.
