...
const ApproversTable: React.FC<ApproversTableProps> = ({ search }) => {
  ...
  const searchResult = useSearchMintingUsersQuery(
    { query: search, role: "approver", page: currentPage, page_size: pageSize },
    { skip: !isSearching },
  );
  const listResult = useGetMintingUsersQuery(
    { role: "approver", page: currentPage, page_size: pageSize },
    { skip: isSearching },
  );
  ...
  if (isLoading) {
    return <UserName>Loading Approvers...</UserName>;
  }
