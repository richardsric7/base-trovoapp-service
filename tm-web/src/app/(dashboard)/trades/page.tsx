...
import TradesFilter from "../trovop2p/components/TradesFilter";
  ...
  return (
    <Container>
      <TitleSection>
        <Title>Trades</Title>
        <TotalCount>
          Total Trades: <span>{data?.data?.total}</span>
        </TotalCount>
      </TitleSection>
      <HeaderContainer>
        <SearchBar
      ...
      {isLoading ? (
        ...
        <>
          <UserName>Searching trades...</UserName>
        </>
      ) : searchInput && data?.data?.total === 0 ? (
        <>
          <UserName>No trades match your search.</UserName>
        </>
      ) : (
        <>
