...
const CustodianTokenizedAssetPage = () => {
      ...
      {custodianAssets.length === 0 ? (
        ...
        <Container>
          ...
          <TableContainer>
            <CustomTable
              columns={columns}
              dataSource={custodianAssets}
              isLoading={false}
              onRowClick={(record) => {
                router.push(
                  `/organisation/assetcustodian/tokenizedasset/${record.id}`,
                );
              }}
            />
            <Pagination
...
export default CustodianTokenizedAssetPage;
