...
const TokenizedAssetDetailsPage = ({
  ...
  const asset = {
    ...
    assetStatus:
      TOKENIZATION_STATUS_LABELS[data?.assetTokenizationStatus ?? 0] ??
      "Unknown",
    ...
    salesStartDate: data?.offering?.sales_start
      ? new Date(data.offering.sales_start).toLocaleDateString()
      : "N/A",
    salesEndDate: data?.offering?.sales_end
  ...
  const handleMenuClick: MenuProps["onClick"] = ({ key }) => {
    switch (key) {
      case "1": // Primary Sale Details
        router.push(
          "/organisation/issuinghouse/tokenizedasset/primarysalesdetails",
        );
        break;
      case "2": // Commitment Details
        router.push(
          "/organisation/issuinghouse/tokenizedasset/commitment-details",
        );
        break;
      case "3": // View Token Holders
        router.push("/organisation/issuinghouse/tokenizedasset/tokenholders");
        break;
      case "4": // Early Exit
        router.push("/organisation/issuinghouse/tokenizedasset/earlyexit");
        break;
  ...
  const tokenSales = [
    ...
    {
      label: "Cap Duration",
      value: data?.offering ? `${data.offering.cap_duration_days} days` : "N/A",
    },
    {
      ...
      date:
        stage.status_id === 0 && data.tokenization_timeline?.date_submitted
          ...
          : stage.current && data.tokenization_timeline?.date_approved
            ? new Date(
                data.tokenization_timeline.date_approved,
              ).toLocaleString()
            : "",
    })) ?? [];
        ...
        <Section>
          <HeadingContent>
            ...
            <ButtonContainer>
              ...
              // onClick={() => {
              //   router.push("/organisation/issuinghouse/tokenizedasset/dividendandyield");
              // }}
            >
              Authorize
            </PrimaryButton> */}
