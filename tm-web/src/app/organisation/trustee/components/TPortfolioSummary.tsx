...
import { TrusteePortfolioSummary } from "@/redux/api/trustees";

const Chart = dynamic(() => import("react-apexcharts"), { ssr: false });

interface PortfolioSummaryProps {
  summary?: TrusteePortfolioSummary;
}

const TPortfolioSummary = ({ summary }: PortfolioSummaryProps) => {
  const categories = summary?.categories || [];
  const series = categories.map((cat) => cat.asset_count ?? 0);
  const labels = categories.map((cat) => cat.category || "Uncategorized");
  const totalAssets = summary?.asset_count ?? categories.reduce((total, cat) => total + (cat.asset_count ?? 0), 0);
          ...
          <List>
            {categories.map((cat, index) => {
              ...
              const valText = mainVal
                ? `${mainVal.currency || ""} ${Number(mainVal.amount || 0).toLocaleString()}`
...
export default TPortfolioSummary;
