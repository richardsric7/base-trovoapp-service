...
import PayYieldModal from "./PayYieldModal";
...
const YieldTable = () => {
      ...
      render: (_: any, record: any) => {
        return (
          <Actionwrapper
            onClick={() =>
              router.push(
                `/organisation/assetmanager/tokenizedasset/dividendandyield/${record.id}`
              )
            }
          >
            View
  ...
  return (
    <Container>
      <Header>
        <Title>Yield Payout History</Title>
        <PrimaryButton
      ...
      <PayYieldModal isOpen={isPay} onClose={() => setIsPay(false)} />
    </Container>
  );
};
