...
interface RecentAssetAssignedProps {
  assets?: FaRecentAsset[];
}

const FaRecentAssetAssigned = ({ assets = [] }: RecentAssetAssignedProps) => {
      ...
      render: (_: any, record: any) =>
        record.status === "Pending Structuring" ? (
          <ActionButton
            onClick={() =>
              router.push(
                `/organisation/financialadviser/structuring/${record.key}/complete`,
              )
            }
          >
...
export default FaRecentAssetAssigned;
