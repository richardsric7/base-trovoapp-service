...
import { useUpdateSharedAccessMutation } from '../../../store/api/sharedAccessApis';
import { showNotification } from '../../../utils/showToaster';
import { parsePermissionInput } from './utils';

export default function SharedAccessUpdate() {
  const navigate = useNavigate();
  const appUser = useSelector((state: RootState) => state.auth.user!);
  const [updateSharedAccess] = useUpdateSharedAccessMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if ('data' in response && response.data) {
        showNotification('success', 'Shared access update request submitted');
        navigate('/dashboard/shared-access');
      } else {
        showNotification('error', 'Unable to update shared access');
      }
    } catch (error) {
      showNotification('error', 'Unable to update shared access');
    } finally {
    ...
    <div className="flex flex-col gap-5 p-4 md:p-6">
      ...
      <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-500">
          Adjust the viewers, approvers, and initiators for the selected wallet.
        </p>
