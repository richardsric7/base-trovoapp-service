...
func BlockchainAccountProperties(client *horizonclient.Client, destinationPublicKey string, asset txnbuild.Asset) (accountExists bool, trustsAsset bool, nativeBalance decimal.Decimal, assetBalance decimal.Decimal, accountObj *horizon.Account, err error) {
	...
	if destinationHorizonError != nil {
		...
		if !destinationAccountExists { // no lint
...
func SubmitXdrWithSignature(client *horizonclient.Client, signerPublicKey string, xdrBase64 string, signature string) (string, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "timeout") || strings.Contains(err.Error(), "handshake") || strings.Contains(err.Error(), "read tcp") || strings.Contains(err.Error(), "connection reset by peer") || strings.Contains(err.Error(), "dial tcp") || strings.Contains(err.Error(), "no such host") {
			err = discord.Say(fmt.Sprintf("[SubmitXdrWithSignature] error connecting to expansion service: %v\nXDR: %v", err, xdrBase64))
			if err != nil {
				return "", err
			}
		}
...
func SubmitApprovalsXdrWithSignatures(client *horizonclient.Client, approvalID string, db *gorm.DB) (string, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "timeout") || strings.Contains(err.Error(), "handshake") || strings.Contains(err.Error(), "read tcp") || strings.Contains(err.Error(), "connection reset by peer") || strings.Contains(err.Error(), "dial tcp") || strings.Contains(err.Error(), "no such host") {
			err = discord.Say(fmt.Sprintf("[SubmitApprovalXdrWithSignature] error connecting to expansion service: %v\nXDR: %v", err, xdrBase64))
			if err != nil {
				return "", err
			}
		}
