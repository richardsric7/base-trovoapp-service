...
type PaymentLog struct {
	...
	ContractAddress            *string  `gorm:"size:56;null"`      //source asset
	AssetCode                  *string  `gorm:"size:12;null"`      //source asset
	DestinationContractAddress *string  `gorm:"size:56;null"`      //for swap
...
// 	}
// 	var result Result
// 	gc.DB.Raw("SELECT id FROM Tokenized_Assets WHERE Asset_Tokenization_Status > 4 AND Asset_Code = upper(?) AND Issuing_Wallet_Public_Key = upper(?)", s.DestinationAssetCode, s.DestinationContractAddress).Scan(&result)
