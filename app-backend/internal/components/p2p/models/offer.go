...
type Offer struct {
	...
	// PaymentMethod is a denormalized display copy of the SELL offer's
	// linked MerchantPaymentMethod (PaymentMethodID), kept in sync whenever
	// that record is edited - see services.UpdatePaymentMethod. Unset for
	// BUY offers.
	PaymentMethod   PaymentMethod `json:"paymentMethod" gorm:"embedded;embeddedPrefix:payment_method_"`
	PaymentMethodID string        `json:"paymentMethodId" gorm:"size:36;index:idx_p2p_offer_payment_method"`
	// MerchantPayoutAddress is the merchant's own wallet/subwallet address
	// to receive the asset on a BUY offer (Plan: "the wallet they specify
	// to use to receive their asset"). Unset for SELL offers.
	MerchantPayoutAddress string    `json:"merchantPayoutAddress" gorm:"size:56;not null;default:''"`
