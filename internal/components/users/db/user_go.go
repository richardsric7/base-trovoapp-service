...
func GetUser(userInfo string, db *gorm.DB, gc *sharedconfig.GlobalConfig) (user userModels.User, err error) {
	...
	if len(userInfo) == 56 {
		...
		e = db.Preload("UserWallets.Permissions").Preload(clause.Associations).Where("id = ? OR username = ? OR mobile = ? OR email = ?", userInfo, strings.ToLower(userInfo), &userInfo, userInfo).First(&user).Error
	}
