...
func CreateNewSubWallet(user *userModels.User, subWalletInfo *userModels.SubWalletInfo, gc *sharedconfig.GlobalConfig) (*userModels.SubWalletInfo, error) {
	...
	if len(subWalletInfo.TransactionSignature) == 0 {
	...
	if len(subWalletInfo.ChannelAccountSignature) > 0 && len(subWalletInfo.ChannelAccount) == 56 {
		...
		txnHash, err := SubmitSubWalletXdrWithSignature(client, user.PublicKey, subWalletInfo.PublicKey, xdrBase64, subWalletInfo.TransactionSignature)
		if err != nil {
...
func SubmitSubWalletXdrWithSignature(client *horizonclient.Client, ownerPublicKey, subWalletPublicKey string, xdrBase64 string, signature string) (string, error) {
